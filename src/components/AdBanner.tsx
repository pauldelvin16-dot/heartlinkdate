...
export function AdBanner({ onReward }: { onReward?: (n: number) => void }) {
  const { user } = useAuth();
  ...
  useEffect(() => {
    (async () => {
      const [{ data: site }, { data }] = await Promise.all([
        ...
        supabase.from("ads").select("*").eq("is_active", true).order("weight", { ascending: false }),
      ]);
      setAdsEnabled(site?.ads_enabled !== false);
      const now = new Date();
      const list = ((data ?? []) as Ad[]).filter((a: any) =>
  ...
  const banner = ads.find(a => a.placement === "banner" && !hiddenIds.has(a.id) && !dismissedToday(a.id));
  const floating = ads.find(a => a.placement === "floating" && !hiddenIds.has(a.id) && !dismissedToday(a.id));
      ...
      {banner && (
        <motion.div initial={{ y: -8, opacity: 0 }} animate={{ y: 0, opacity: 1 }}
          ...
          <div className="flex items-center gap-3 p-3">
            {banner.image_url && <img src={banner.image_url} alt="" className="h-14 w-14 rounded-lg object-cover" />}
            {banner.video_url && (
            ...
            <button onClick={() => { dismissAd(banner.id); setHiddenIds(s => new Set([...s, banner.id])); }} className="text-muted-foreground hover:text-foreground">
              <X className="h-4 w-4" />
            </button>
          </div>
        </motion.div>
        ...
        {floating && (
          <motion.div initial={{ scale: 0, y: 40 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0, opacity: 0 }}
            ...
            <div className="relative max-w-[260px] rounded-2xl border border-primary/40 bg-card p-3 shadow-glow">
              <button onClick={() => { dismissAd(floating.id); setHiddenIds(s => new Set([...s, floating.id])); }}
                className="absolute -right-2 -top-2 grid h-6 w-6 place-items-center rounded-full border border-border bg-card text-muted-foreground shadow-soft hover:text-foreground">
                <X className="h-3 w-3" />
              </button>
              {floating.video_url && (
                <video src={floating.video_url} muted loop autoPlay playsInline className="mb-2 h-28 w-full rounded-lg object-cover" />
              )}
              {!floating.video_url && floating.image_url && (
                <img src={floating.image_url} alt="" className="mb-2 h-24 w-full rounded-lg object-cover" />
              )}
              ...
              <p className="mt-1 text-center text-[10px] text-muted-foreground">Watch to earn — skip anytime</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
